...
unsafe extern "C" {
    ...
    fn micro_esp_ui_create_tabview(node: u32, parent: u32, titles: *const u8, titles_len: usize) -> c_int;
    fn micro_esp_ui_create_tab_content(index: u32) -> c_int;
...
impl NativeUi for EspNativeUi {
    ...
    fn create_tabview(
        ...
        native_result(unsafe {
            micro_esp_ui_create_tabview(node.0, parent_id(parent), joined.as_ptr(), joined.len())
        })
    }

    fn create_tab_content(&mut self, index: u32) -> Result<(), String> {
        native_result(unsafe { micro_esp_ui_create_tab_content(index) })
    }
